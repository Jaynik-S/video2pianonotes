...
The shared default settings live in `video2midi/.v2m.ini`. You can still save per-video sidecar settings next to the source media as `<video>.ini`.
...
Базовый конфиг лежит в `video2midi/.v2m.ini`, а настройки конкретного ролика по-прежнему можно сохранять рядом с исходным видео как `<video>.ini`.
